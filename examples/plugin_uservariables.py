...
#
"""
<plugin key="xfr_api_uservars" name="Demo Uservariables" author="Xorfor" version="1.0.0">
    <params>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        self.__var = dom.UserVariable(
            self.__server,
            "API Test Uservariable",
            dom.UserVariable.UVE_TYPE_STRING,
            " ")
        if not self.__var.exists():
            Domoticz.Debug("API Test Uservariable does NOT exists")
            self.__var.add()
        else:
            Domoticz.Debug("API Test Uservariable exists!!!")
        # DumpAllToLog()
    ...
    def onHeartbeat(self):
        ...
        Domoticz.Debug("self.__server: {}".format(self.__server))
        Domoticz.Debug("self.__var: {}".format(self.__var))
        self.__runAgain -= 1
        # Update uservariable with the `__runAgain` value
        self.__var.value = self.__runAgain
